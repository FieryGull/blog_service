use crate::{
    common_lib::{error_handler::CustomError, jwt_auth::JwtMiddleware},
    posts::{NewPost, Posts},
};
...
async fn find_all(user_data: JwtMiddleware) -> Result<HttpResponse, CustomError> {
    let posts = web::block(move || Posts::find_all(user_data.user_id))
        .await
        .unwrap();
...
#[get("/posts/{id}")]
async fn find(id: web::Path<Uuid>, user_data: JwtMiddleware) -> Result<HttpResponse, CustomError> {
...
async fn delete(
    ...
    match deleted_post {
        0 => Err(CustomError::new(
            404,
            "The query object not found".to_string(),
        )),
